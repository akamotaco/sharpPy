...
namespace SharpPy.PegGenerator.CodeGenerator
{
    ...
    public partial class CSharpCodeGenerator
    {
        ...
        private void GenerateIfStatementParser()
        {
            ...
            WriteLine("var bodyStmt = ParseStatement();");
            ...
            WriteLine("var elifStmt = ParseStatement();");
            ...
            WriteLine("var elseStmt = ParseStatement();");
        ...
        private void GenerateBasicMethods()
        {
            ...
            WriteLine();
            WriteLine("var args = new List<object>();");
            WriteLine("var defaults = new List<object>();");
            WriteLine("var startPos = _position;");
            WriteLine();
            ...
            WriteLine();
            WriteLine("// Check for default value assignment (e.g., \"y=10\")");
            WriteLine("Console.WriteLine($\"[DEBUG] ParseParameters: After NAME '{nameToken.Value}', current token: {CurrentToken?.Type}('{CurrentToken?.Value}') at position {_position}\");");
            WriteLine("if (CurrentToken != null && CurrentToken.Type == GeneratedTokenType.OP && CurrentToken.Value == \"=\")");
            WriteLine("{");
            WriteLine("    Console.WriteLine($\"[DEBUG] ParseParameters: found '=' for default value at position {_position}\");");
            WriteLine("    Advance(); // Move past '='");
            WriteLine();
            WriteLine("    // Parse the default value expression");
            WriteLine("    var defaultExpr = ParseExpression();");
            WriteLine("    if (defaultExpr != null)");
            WriteLine("    {");
            WriteLine("        defaults.Add(defaultExpr);");
            WriteLine("        Console.WriteLine($\"[DEBUG] ParseParameters: parsed default value for '{nameToken.Value}'\");");
            WriteLine("    }");
            WriteLine("    else");
            WriteLine("    {");
            WriteLine("        Console.WriteLine($\"[DEBUG] ParseParameters: failed to parse default value for '{nameToken.Value}'\");");
            WriteLine("        _position = startPos;");
            WriteLine("        return null;");
            WriteLine("    }");
            WriteLine("}");
            ...
            WriteLine("{");
            WriteLine("    Console.WriteLine($\"[DEBUG] ParseParameters: parsed {args.Count} arguments with {defaults.Count} defaults\");");
            WriteLine("    // Call _PyPegen_make_arguments(p, posonlyargs, posonly_defaults, args, defaults, star_etc)");
            WriteLine("    var result = _PyPegen_make_arguments(this, null, null, args, defaults.Count > 0 ? defaults : null, null);");
            ...
            WriteLine("        {");
            WriteLine("            Console.WriteLine($\"[DEBUG] ParseClassDef: Unexpected token in base classes: {CurrentToken?.Type}:{CurrentToken?.Value}\");");
            WriteLine("            break;");
            WriteLine("        }");
            WriteLine("    }");
            WriteLine("    if (CurrentToken?.Type == GeneratedTokenType.OP && CurrentToken?.Value == \")\")");
            ...
            WriteLine("}");
            WriteLine();
            WriteLine("// Parse except clauses");
            ...
            WriteLine("    if (CurrentToken?.Type != GeneratedTokenType.OP || CurrentToken?.Value != \":\")");
            WriteLine("    {");
            WriteLine("        Console.WriteLine($\"[DEBUG] ParseTryStatement: Expected ':' after except but found {CurrentToken?.Type}:{CurrentToken?.Value}\");");
            WriteLine("        return null;");
            WriteLine("    }");
            WriteLine("    Advance(); // consume ':'");
            WriteLine();
            ...
            WriteLine("    {");
            WriteLine("        Console.WriteLine($\"[DEBUG] ParseTryStatement: Failed to parse except body\");");
            WriteLine("        return null;");
            WriteLine("    }");
            WriteLine();
            WriteLine("    exceptClauses.Add(new { type = exceptionType, name = exceptionName, body = exceptBody });");
            WriteLine("}");
            WriteLine();
            ...
            WriteLine("if (CurrentToken?.Type == GeneratedTokenType.OP && CurrentToken?.Value == \"*\")");
            WriteLine("{");
            WriteLine("    importNames.Add(new { name = \"*\", asname = \"\" });");
            WriteLine("    Advance();");
            WriteLine("}");
            WriteLine("else");
            WriteLine("{");
            ...
            WriteLine("        }");
            WriteLine();
            WriteLine("        importNames.Add(new { name = importName, asname = asName });");
            WriteLine();
            WriteLine("        // Check for comma separator");
            WriteLine("        if (CurrentToken?.Type == GeneratedTokenType.OP && CurrentToken?.Value == \",\")");
            WriteLine("        {");
        ...
        private void GenerateDecoratorParsers()
        {
            ...
            WriteLine("}");
            WriteLine();
            WriteLine("// Parse case body according to grammar: block = NEWLINE INDENT statements DEDENT");
            WriteLine("if (CurrentToken?.Type == GeneratedTokenType.NEWLINE)");
            WriteLine("{");
            WriteLine("    // Consume NEWLINE");
            WriteLine("    Advance();");
            WriteLine("    ");
            WriteLine("    // Consume INDENT");
            WriteLine("    if (!ExpectToken(GeneratedTokenType.INDENT))");
            WriteLine("    {");
            WriteLine("        Console.WriteLine($\"[DEBUG] ParseCaseBlock: Failed to match INDENT after NEWLINE at position {_position}\");");
            WriteLine("        _position = startPos;");
            WriteLine("        return null;");
            WriteLine("    }");
            WriteLine("    ");
            WriteLine("    // Parse statements within the case body");
            WriteLine("    var statements = new List<object>();");
            WriteLine("    while (CurrentToken?.Type != GeneratedTokenType.DEDENT && CurrentToken?.Type != GeneratedTokenType.ENDMARKER)");
            WriteLine("    {");
            WriteLine("        var stmt = ParseStatement();");
            WriteLine("        if (stmt != null)");
            WriteLine("        {");
            ...
            WriteLine("    }");
            WriteLine("    ");
            WriteLine("    // Consume DEDENT (end of case body)");
            WriteLine("    if (!ExpectToken(GeneratedTokenType.DEDENT))");
            WriteLine("    {");
            WriteLine("        Console.WriteLine($\"[DEBUG] ParseCaseBlock: Failed to match DEDENT at end of case body at position {_position}\");");
            WriteLine("        _position = startPos;");
            WriteLine("        return null;");
            WriteLine("    }");
            WriteLine("    ");
            WriteLine("    Console.WriteLine($\"[DEBUG] ParseCaseBlock: Successfully parsed case block with {statements.Count} statements\");");
            WriteLine("    return new { pattern = pattern, guard = (object)null, body = statements };");
            WriteLine("}");
            WriteLine("else if (CurrentToken?.Type == GeneratedTokenType.NAME && CurrentToken.Value == \"pass\")");
            WriteLine("{");
            WriteLine("    // Simple pass statement");
            WriteLine("    Advance(); // consume 'pass'");
            WriteLine("    Console.WriteLine($\"[DEBUG] ParseCaseBlock: Successfully parsed simple pass statement\");");
            WriteLine("    return new { pattern = pattern, guard = (object)null, body = \"pass\" };");
            WriteLine("}");
            WriteLine();
