...
namespace SharpPy.PegGenerator.CodeGenerator
{
    ...
    public partial class CSharpCodeGenerator
    {
        ...
        private void GenerateBasicMethods()
        {
            ...
            WriteLine("{");
            WriteLine("    Advance(); // consume '*'");
            WriteLine("    // Simplified star_etc parsing");
            WriteLine("    var result = new Dictionary<string, object>();");
            WriteLine("    result[\"vararg\"] = \"*args\"; // Placeholder");
            ...
            WriteLine("{");
            WriteLine("    Advance(); // consume '**'");
            WriteLine("    var kwarg = ParseParam();");
            WriteLine("    var result = new Dictionary<string, object>();");
            WriteLine("    result[\"vararg\"] = null;");
            WriteLine("    result[\"kwonlyargs\"] = new List<object>();");
            WriteLine("    result[\"kwarg\"] = kwarg;");
            WriteLine("    return result;");
            WriteLine("}");
            WriteLine();
            WriteLine("return null;");
            ...
            WriteLine("        return statements;");
            WriteLine();
            WriteLine("    // Parse statements in the indented block - CPython 3.12: return statements directly, no flattening");
            WriteLine("    var blockStatements = ParseStatements();");
            WriteLine("    if (blockStatements != null)");
            WriteLine("    {");
            WriteLine("        // CPython 3.12: Return block statements as-is, do not flatten");
        ...
        private void GenerateImprovedParseAssignmentMethod()
        {
            WriteLine();
            WriteLine("/// <summary>");
            WriteLine("/// Enhanced ParseAssignment method with augmented assignment support");
            WriteLine("/// Handles both regular assignment (=) and augmented assignment (+=, -=, etc.)");
            WriteLine("/// </summary>");
            WriteLine("public GeneratedStmt ParseAssignmentEnhanced()");
            WriteLine("{");
            Indent();
            ...
            WriteLine("return _PyAST_AugAssign(augTarget, augOp, value);");
            Dedent();
            WriteLine("}");
            Dedent();
            WriteLine("}");
