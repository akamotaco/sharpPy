...
namespace SharpPy.PegGenerator.Asdl
{
    public class PyParserBaseGenerator
    {
        ...
        private void GenerateParserBaseClass()
        {
            ...
            WriteLine("Console.WriteLine($\"[LR] {ruleName}: Starting at pos={_position}, seeding with FAIL\");");
            WriteLine("#endif");
            ...
            WriteLine("#if DEBUG_PARSE_LOG");
            WriteLine("Console.WriteLine($\"[LR] {ruleName}: First attempt result={(result == null ? \"null\" : \"not-null\")}, pos={_position}\");");
            WriteLine("#endif");
            WriteLine();
            ...
            _indentLevel++;
            WriteLine("#if DEBUG_PARSE_LOG");
            WriteLine("Console.WriteLine($\"[LR] {ruleName}: Growth attempt, resetting to pos={key.Item1}\");");
            WriteLine("#endif");
            WriteLine("// Reset to start position for next growth attempt");
            WriteLine("_position = key.Item1;");
            WriteLine("var newResult = ruleFunc();");
            WriteLine("#if DEBUG_PARSE_LOG");
            WriteLine("Console.WriteLine($\"[LR] {ruleName}: Growth result={(newResult == null ? \"null\" : \"not-null\")}, pos={_position}, lastEndPos={lastEndPos}\");");
            WriteLine("#endif");
            WriteLine();
            ...
            _indentLevel++;
            WriteLine("#if DEBUG_PARSE_LOG");
            WriteLine("Console.WriteLine($\"[LR] {ruleName}: Terminating, returning lastResult at pos={lastEndPos}\");");
            WriteLine("#endif");
            ...
            WriteLine();
            WriteLine("#if DEBUG_PARSE_LOG");
            WriteLine("Console.WriteLine($\"[LR] {ruleName}: Growth succeeded, updating seed\");");
            WriteLine("#endif");
